...
Page({
  ...
  submit: function () {
    ...
    var qt = new Object();
    qt.RegType = RegType1
    qt.Institutional = this.data.Institutional_items[this.data.Institutional_index]
    qt.InsState = this.data.InsState
      ...
      getData.req("collection/ctg_wares.jspx", "POST", { key: app.globalData.key, id: this.jc.id }, res => {
        if (res.data.status == 200) {
          ...
          this.setData({
            ...
            Institutional_index: positionStr.radio_position(this.data.Institutional_items, this.data.Institutional),
          })
        }
      })
    }
